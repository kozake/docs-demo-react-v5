...
const InfiniteScroll: React.FC = () => {
  const [items, setItems] = useState(users.slice(0, 15));
  ...
  const handleInfinite = useCallback(async () => {
    ...
    const newUsers = [];
    for (let i = 0; i < 5; i++) {
      newUsers.push(users[Math.floor(Math.random() * users.length)]);
    }
    setItems([...items, ...newUsers]);
    infiniteScrollEl.current?.complete();
  }, [items]);

  return (
    <IonPage className="page-infiniteScroll">
      <IonHeader translucent={true}>
        <IonToolbar>
          <IonButtons slot="start">
            <IonBackButton defaultHref="/"></IonBackButton>
          </IonButtons>
          <IonTitle>Infinite Scroll</IonTitle>
        </IonToolbar>
      </IonHeader>
            ...
            return (
              <IonItem key={i}>
                <IonAvatar slot="start">
                  <img
                    src={`https://www.gravatar.com/avatar/${i}?d=monsterid&f=y`}
                    alt="avatar"
                  />
                </IonAvatar>
                <IonLabel>
